...
function processFrontEndMessage(client, message) {
    const dataMDiscord = toHTML(message.content, {
        discordCallback: {
            user: node => {
                return '@' + (message.guild.members.cache.get(node.id) ? message.guild.members.cache.get(node.id).displayName : (client.users.cache.get(node.id) ? client.users.cache.get(node.id).username : "Deleted User"));
            },
            channel: node => {
                return '#' + (message.guild.channels.cache.get(node.id) ? message.guild.channels.cache.get(node.id).name : "deleted-channel");
            },
            role: node => {
                return '@' + (message.guild.roles.cache.get(node.id) ? message.guild.roles.cache.get(node.id).name : "deleted-role");
            }
        },
        escapeHTML: true
    })
    ...
    if (validInvs(emojiFind)) emojiFind = `**${message.author.username}** invalid link.`;
